...
LOG_CALL_RE = re.compile(r'\bLOG_(ERR|WRN|INF|DBG)\s*\(')
...
def generate(config, old_map):
    modules = config.get('modules', {})
    scanned = scan_c_files(modules)              # path -> module name
    ...
    for fid in sorted(files):
        if files[fid]["present"]:
            entries.extend(extract_entries(files[fid]["path"], fid))
    entries.sort(key=lambda e: (e["file_id"], e["line"]))
...
def emit_header(config, the_map):
    ...
    lines = [
        ...
        " * Generated by tools/gen_log_map.py from log_config.json + ww_log_map.json",
        " */",
        "",
...
def main():
    ...
    new_map = generate(config, old_map)
